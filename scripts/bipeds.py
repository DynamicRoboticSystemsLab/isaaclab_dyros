...
def run_simulator(sim: sim_utils.SimulationContext, robots: list[Articulation], origins: torch.Tensor):
    ...
    while simulation_app.is_running():
        ...
        for robot in robots:
            # feet_idx, feet_name = robot.find_bodies("L_AnkleRoll_Link")
            # print(robot.data.body_link_pose_w[:, feet_idx].to("cpu").numpy()[0,0,:3])
            # print(robot.data.root_pose_w.to("cpu").numpy()[0,2])
